...
`feed.ts`, and `cairn.server.ts` carry the same shape but wire ASC's own three concepts, `posts`,
...
| File | What it is |
| --- | --- |
| `content.ts` | The delivery content layer: globs the markdown, builds the site/posts/pages/notifications indexes through `createSiteIndexes`. |
