...
// `waiver_text_version` (Task 8's reader, `getWaiverTextVersion`) retired with the rest of the
// pre-T2 waiver machinery (member-waivers T5a): the per-document model (`$theme/documents.ts`)
// tracks a signature's wording per document/version/season instead of one global string, and
// migration 0031 drops the row. `getCurrentSeason` is also the season rollover's
