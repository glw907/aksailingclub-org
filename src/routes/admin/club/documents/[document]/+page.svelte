...
{:else}
  <div class="mb-4">
    <a class="type-body text-muted hover:text-primary hover:underline" href="/admin/club/documents?season={data.season}">&larr; Back to the season rollup</a>
    <h1 class="mt-1 type-title font-bold">{data.summary.title}</h1>
    <p class="type-body text-muted">Version {data.summary.version} &middot; {data.season} season</p>
  </div>
  ...
  <div class="grid gap-section lg:grid-cols-2">
    <OfficeList title="Outstanding" subtitle="{data.summary.outstanding.length} still owed.">
      {#if data.summary.outstanding.length === 0}
        <p class="px-6 py-6 type-body text-muted">Nothing outstanding.</p>
      {:else}
        <ul class="list">
          {#each data.summary.outstanding as row (row.personId)}
            <li class="list-row items-center">
              <div class="list-col-grow">
                ...
                <a class="ml-2 type-body text-muted hover:text-primary hover:underline" href="/admin/club/members/{row.householdId}">household</a>
              </div>
            </li>
          {/each}
        </ul>
    ...
    <OfficeList title="Signed" subtitle="{data.summary.signed.length} on file.">
      {#if data.summary.signed.length === 0}
        <p class="px-6 py-6 type-body text-muted">No signature yet.</p>
      {:else}
        <ul class="list">
          {#each data.summary.signed as row (row.personId)}
            <li class="list-row items-center">
              <div class="list-col-grow">
                <a class="font-medium hover:text-primary hover:underline" href="/admin/club/documents/member/{row.personId}">{row.personName}</a>
                {#if row.signature}
                  <p class="type-body text-muted">Signed {formatCivilDate(row.signature.signedAt)}</p>
                {/if}
              </div>
              {#if row.signature}
