...
{:else}
  <div class="mb-4">
    <a class="type-body text-muted hover:text-primary hover:underline" href="/admin/club/members/{data.member.householdId}">&larr; Back to the household</a>
    <h1 class="mt-1 type-title font-bold">{data.member.name}</h1>
    <p class="type-body text-muted">Signature history</p>
  </div>

  <OfficeList eyebrow="Club" title="Signatures" subtitle="{data.history.length} on file.">
    {#if data.history.length === 0}
      <p class="px-6 py-6 type-body text-muted">No signature on file yet.</p>
    {:else}
      <table class="table">
        <thead>
          <tr>
            ...
            <tr>
              <td>
                ...
                {#if row.onBehalfOfMinorName}
                  <span class="ml-1 type-body text-muted">for {row.onBehalfOfMinorName}</span>
                {/if}
              </td>
              <td>{row.season}</td>
              <td class="type-body text-muted">{CONTEXT_LABEL[row.context] ?? row.context}</td>
              <td class="type-body text-muted">{formatCivilDate(row.signedAt)}</td>
