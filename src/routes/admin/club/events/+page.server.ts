 ...
 *  `events-store.ts` for a series move, duplicated here as a direct `db.batch` (rather than a
 *  second store export) because `deleteEvent` itself is a single, non-batched statement and the
 *  `delete` action needs the two writes atomic. */
